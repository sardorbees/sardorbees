<h1 align="center">Привет 👋, я Rizayev Sardor</h1>

<h3 align="center">
🚀 Frontend разработчик | Vue • Nuxt • Next • Django
</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=36BCF7&center=true&vCenter=true&lines=Frontend+Developer;Vue+%7C+Nuxt+%7C+Next.js;Python+Django+Developer;Telegram+Bot+Developer" />
</p>

---

## 🌐 Обо мне

- 🚀 Работаю с: Vue, Nuxt.js, Next.js  
- ⚡ Использую: SSR / SSG  
- 🐍 Backend: Python + Django  
- 🤖 Разрабатываю Telegram ботов  
- 🌐 Сайт: https://ali-company.vercel.app/  

---

## 🛠️ Технологии

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,ts,react,nextjs,vue,nuxtjs,python,django,git,github,vscode" />
</p>

---

## 🚀 Мои проекты

- 🌐 **Portfolio Website**  
  https://ali-company.vercel.app/

- 🛒 **E-commerce (React + Django)**  
  Онлайн магазин с оплатой (Click/Payme)

- 🤖 **Telegram Bot**  
  Бот с уведомлениями и автоматизацией

---

## 📊 GitHub статистика

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=sardorbees&show_icons=true&theme=tokyonight" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sardorbees&layout=compact&theme=tokyonight" />
</p>

---

- 🛒 **E-commerce Platform (React + Django)**  
  Полноценный онлайн-магазин с современной архитектурой:

  🔐 Аутентификация:
  - Google OAuth (вход через Google)
  - SMS OTP (подтверждение по коду)
  - JWT (access + refresh)

  💳 Оплата:
  - Click / Payme интеграция
  - Webhook подтверждение платежа

  ⚙️ Backend:
  - Django REST API
  - Защита пользователей и токенов

  🎯 Frontend:
  - React + Axios
  - Защищённые маршруты

  🚀 Features:
  - Регистрация / Логин
  - Корзина и оформление заказа
  - Уведомления (Telegram)

---

## 🔥 Активность

<p align="center">
  <img src="https://streak-stats.demolab.com?user=sardorbees&theme=tokyonight" />
</p>

---

  🔥 PRO Features:
  - Social Auth (Google, GitHub)
  - SMS через Eskiz / PlayMobile
  - Rate limiting + защита от ботов
  - Email + Telegram уведомления
  - Админ панель (Dashboard)
    

--- 

## 📫 Связь

<p align="center">
  <a href="https://t.me/ali_equity">
    <img src="https://img.shields.io/badge/Telegram-Написать-2CA5E0?style=for-the-badge&logo=telegram&logoColor=white"/>
  </a>
  <a href="https://instagram.com/websardor">
    <img src="https://img.shields.io/badge/Instagram-Профиль-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
  </a>
  <a href="https://ali-company.vercel.app/">
    <img src="https://img.shields.io/badge/🌐_Сайт-Портфолио-black?style=for-the-badge"/>
  </a>
</p>

---

## 🧑‍💻 Hire Me

<p align="center">
  <a href="https://t.me/ali_equity">
    <img src="https://img.shields.io/badge/Hire_Me-Работать_со_мной-success?style=for-the-badge"/>
  </a>
</p>

---

## ⚡ О себе

💡 Каждый день пишу код и развиваюсь  
🔥 Люблю чистую архитектуру и современный UI  

---

## 🐍 GitHub Snake

![snake gif](https://github.com/sardorbees/sardorbees/blob/output/github-contribution-grid-snake.svg)
